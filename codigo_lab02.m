...
colors = ['r', 'g', 'b', 'k'];    % Colores para distinguir cada α


%% --- Gráficos de frecuencia e impulso ---
figure;
...
for i = 1:length(alphas)
    ...
    impulsos{i} = he;  % Guardamos respuesta para más tarde
...
ylabel('Amplitud');
legend('show'); grid on;
% ----- Fin de gráficos de frecuencia e impulso -----
%% --- Diagrama de Ojo ---
% Parámetros para el diagrama de ojo
...
colors = ['b', 'b', 'b', 'b'];  % Colores para cada α
...
for idx = 1:length(alpha_values)
    ...
    % Desplazamiento de 0.5 segundos (500 milisegundos)
    offset = 0.5;  % Desplazamiento en segundos
    offset_samples = offset * fs;  % Convertir el desplazamiento a muestras
    % Graficar los segmentos con desplazamiento
    for k = 1:min(50, num_segments)
        ...
        time = ((0:length(segment)-1) / fs) * 1e3;  % Tiempo en ms sin desplazamiento
